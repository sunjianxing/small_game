# serve_addictive_dodge.py
# Run: python serve_addictive_dodge.py
# Then open http://localhost:8000 (the script will try to open it automatically).
...
def write_index(path='index.html'):
    with open(path, 'w', encoding='utf-8') as f:
        f.write(textwrap.dedent(INDEX_HTML).strip())
    print(f"Wrote {path}")

def start_server(port=8000, directory='.'):
    os.chdir(directory)
    handler = http.server.SimpleHTTPRequestHandler
    # Python 3.11+ supports directory param in handler; for compatibility we change cwd above.
...
if __name__ == '__main__':
    ...
    # try to open in browser after short delay to ensure server starts
    port = 8000
    def _open():
        time.sleep(0.6)
    ...
    opener.start()
    start_server(port=port, directory='.')
